...
4. **Skeleton repair** — `_ensure_skeleton` / `_ensure_episode` / `_ensure_metadata` / `_ensure_section` create anything missing: the `# Session …` title, `## Mjukvaruversioner` (seeded from `SOFTWARE_DEFAULTS` when empty), `## Avsnittsinfo`, a `### <episode>` block with the `METADATA_FIELDS` lines per on-disk episode, and the standard `####` sections (`STANDARD_SECTIONS` order). `Bryggpromenader` is only created when notes exist for it.
